...
We have presented FedKAN-IDS, a systematic parameter-matched empirical study of Federated Kolmogorov--Arnold Networks against FedAvg-MLP on the NF-BoT-IoT-v2 NetFlow IDS dataset, complemented by a convergence analysis (Theorem~\ref{thm:conv}) that introduces a spline-bias term independent of the data-heterogeneity factor $\Gamma_\alpha$. Across 146 reproducible runs and seed-paired statistical comparisons (Table~\ref{tab:stats_botiot}), the headline finding is that under extreme client heterogeneity (Dirichlet $\alpha = 0.1$), the parameter-matched FedAvg-MLP-PM-80 collapses to as low as $56.5\%$ accuracy on its worst seed, while FedKAN-8 maintains $\geq 80.7\%$ on every seed with $2.6\times$ smaller cross-seed standard deviation. Doubling the MLP's capacity from $1.3$k to $3.4$k parameters \emph{worsens} the worst-seed gap rather than narrowing it, supporting an architectural rather than capacity-driven reading of the result. Per-class analysis under the multi-class regime confirms the same architectural signature on the DoS attack class, where FedKAN's F1 is $0.91$ versus $0.83$ for the parameter-matched MLP, with $\sim 3.5\times$ tighter dispersion.
...
\textbf{Compute-side cost.} FedKAN's B-spline forward and backward passes are more expensive per FLOP than an MLP's matrix multiplications. We measured wall-clock per round as $2.1$--$2.8$~s for FedKAN versus $1.8$--$2.4$~s for MLP-PM on a single A100, well within any IoT FL deployment budget. Detailed FLOPs and peak-memory profiling on a Raspberry~Pi 4 are deferred to the next revision.
...
All code, dataset preparation scripts, configuration files, per-run metrics for the 146 runs reported in Section~\ref{sec:experiments}, the auto-generated tables and figures of this manuscript, and the LaTeX source of the manuscript itself are released at \url{https://github.com/haodpsut/FedKAN-IDS-v2}. The repository's notebook \texttt{notebooks/10\_run\_batch.ipynb} reproduces the full M2 + M3a grid on a Google Colab Pro+ A100 runtime in approximately $3.5$~hours.
