...
def main():
    ...
    # LaTeX table — table* (full text width) + \small to fit 9 columns.
    tex = [
        r"% Auto-generated by scripts/stats_tests.py --prefix " + args.prefix,
        r"\begin{table*}[t]",
        r"\centering",
        r"\small",
        r"\caption{Statistical comparison on \texttt{" + args.prefix +
        r"}: \textbf{FedKAN-IDS (Ours)} vs FedAvg-MLP-PM (parameter-matched). Mean accuracy (\%), seed-paired difference, percentile-bootstrap 95\% CI of the difference, Cohen's $d$ effect size, and $p$-value from Welch's $t$-test (or paired-$t$ when seeds are paired). $^{*}p{<}0.05$, $^{**}p{<}10^{-2}$, $^{***}p{<}10^{-3}$.}",
