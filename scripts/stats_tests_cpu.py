...
def main():
    ...
    for key, disp, fname, label, pat_bin, pat_mc in CELLS:
        ...
        for mode, pat in (("Binary", pat_bin), ("Multiclass", pat_mc)):
            ...
            if not sp:
                print("  ⛔ %s / %s: khong co du lieu" % (disp, mode))
                continue
            ...
            rows.append((mode, len(sp), st.mean(kk), st.stdev(kk), st.mean(mm), st.stdev(mm),
                         st.mean(d), lo, hi, st.mean(d) / pooled, p))
        if not rows:
            continue
        L = ["%% Auto-generated by scripts/stats_tests_cpu.py -- run CPU, TB 5 vong cuoi",
             "\\begin{table*}[t]", "\\centering",
             ("\\caption{Statistical comparison on %s: FedKAN-8 against the parameter-matched "
              "FedAvg-MLP-PM-80, at the shared $\\eta=10^{-2}$ of the original protocol. "
              "Accuracy is the \\emph{mean over the final five communication rounds}, in \\%%; "
              "$\\Delta$ is the seed-paired mean difference (KAN $-$ MLP), CI is its "
              "percentile-bootstrap 95\\%% interval, $d$ is Cohen's effect size and $p$ is the "
              "paired $t$-test. All runs in this table were executed on the same machine "
              "(%s). Markers: $^{*}p<0.05$, $^{**}p<0.01$, $^{***}p<0.001$.}"
              % (disp, (sorted(devs) or ["?"])[0])),
             "\\label{%s}" % label,
             ...
             "\\midrule"]
        for mo, n, ka, ks, ma, ms, dd, lo, hi, cd, p in rows:
            L.append("%s & Dir($\\alpha{=}0.1$) & %d & %.1f\\,$\\pm$\\,%.1f & %.1f\\,$\\pm$\\,%.1f "
                     "& %+.2f & [%+.1f,\\,%+.1f] & $%+.2f$ & $%.3f%s$ \\\\"
                     % (mo, n, ka, ks, ma, ms, dd, lo, hi, cd, p, stars(p)))
        L += ["\\bottomrule", "\\end{tabular}", "\\end{table*}"]
        (OUT / (fname + ".tex")).write_text("\n".join(L) + "\n", encoding="utf-8")
        print("  da ghi results/tables/%s.tex" % fname)
        for mo, n, ka, ks, ma, ms, dd, lo, hi, cd, p in rows:
            print("     %-11s n=%2d  KAN %.2f+-%.2f  MLP %.2f+-%.2f  D=%+.2f  p=%.3f"
                  % (mo, n, ka, ks, ma, ms, dd, p))

    if len(devs) > 1:
        print("\n  ⛔ CANH BAO: run den tu NHIEU thiet bi: %s" % sorted(devs))
        return 1
    print("\n  ✅ moi run cua ba bang nay tu MOT thiet bi: %s" % (sorted(devs) or ["?"])[0])
    return 0
