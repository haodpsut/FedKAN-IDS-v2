...
\textbf{This failure mode has a literature.} That a comparison can be decided by how well each
side was tuned rather than by the methods themselves is not a new observation, though it appears
not to have reached the federated-KAN line of work. It has been documented in language
modelling~\cite{melis2018eval}, metric learning~\cite{musgrave2020reality}, recommender
systems~\cite{dacrema2019progress}, generative models~\cite{lucic2018gans} and reinforcement
learning~\cite{henderson2018matters}, each time with the same shape: a proposed method is tuned
carefully, the baseline is not, and the reported gain shrinks or vanishes when both receive the
same budget. Related work on reporting practice argues for stating the search budget and the
variance it induces~\cite{dodge2019showwork,bouthillier2021variance}. Our contribution is to
carry that check into federated intrusion detection, where we find the same pattern in our own
