...
\textbf{Single-machine re-run.} Every table in the revised manuscript is now generated from
runs executed on one machine. The headline accuracy table and its last-round companion were
regenerated from scratch rather than carried over, because the submitted version's figures came
from an RTX~4090 while the new experiments ran on CPU, and a table mixing the two would not
support the comparisons we draw from it. The generating script prints the device of every run it
reads and refuses to proceed silently if more than one appears. The submitted numbers came
...
$34.6\%$ are gone and $55.4\%$ of the present one is new, but the automatic comparison marks
only $57\%$ of the new sentences and $45\%$ of the deleted ones. The blue marking is reliable
