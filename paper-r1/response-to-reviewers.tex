...
\usepackage{enumitem}
% Nap nhan tu ban thao de moi \ref o day in ra dung so muc/bang cua BAI, thay vi
% dau hoi. Neu thieu thi phan bien doc "Table ??" va khong tra nguoc duoc, tuc o
% "Where in the manuscript" thanh vo dung.
\usepackage{xr}
...
\where{Abstract; Sec.~\ref{sec:intro} (``Headline finding'' and the four contributions, both rewritten); Sec.~\ref{sec:lrsweep}, which is new; Sec.~\ref{sec:conclusion}. The title changed with them.}
...
\where{Sec.~\ref{sec:algos}, which is new, with Table~\ref{tab:algos} (FedAvg, FedProx, MOON, FedPAQ, SCAFFOLD) and Table~\ref{tab:compound} (the two baseline corrections applied together).}
...
\where{Precision, recall, MCC and false-positive rate are reported in Table~\ref{tab:stats_botiot}, Table~\ref{tab:stats_toniot} and Table~\ref{tab:stats_cseciic}; the derivation and its verification are described in Sec.~\ref{sec:experiments}. The ROC-AUC limitation is stated there too.}
...
schedule. Table~\ref{tab:bytes2target} reports uplink-to-target together with the number of seeds that reach the target at all, which is the column a mean would hide: at the $90\%$ target FedKAN-8 arrives on nine seeds of ten against seven for the parameter-matched MLP, while costing more bytes to get there. Training time and
per-round wall-clock are included, and inference cost is answered under R1.5.
\where{Table~\ref{tab:bytes2target} in Sec.~\ref{sec:cost}, reporting uplink-to-target and the number of seeds that reach the target; per-round wall-clock and round counts are in Sec.~\ref{sec:experiments}.}
...
So that a reader can convert these to their own gateway we also report an anchor on the same machine: a $256\times256$ matrix multiply takes $0.024$~ms. The full protocol, including the anchor, is Table~\ref{tab:hwcost} in the revised Section~\ref{sec:cost}. Parameter parity is not cost
parity, and this now appears in the abstract.
\where{Sec.~\ref{sec:cost} and Table~\ref{tab:hwcost}, including the conversion anchor. The Raspberry~Pi sentences of the submitted version were removed from Sec.~\ref{sec:conclusion} rather than softened.}
...
\where{Sec.~\ref{sec:experiments} (``What this means for the claim'' and the withdrawn skew-gradient passage) and the threats-to-validity list in Sec.~\ref{sec:conclusion}.}
...
has been repaired.
\where{All figures regenerated through a single style module; the repaired conclusion sentence is in Sec.~\ref{sec:conclusion}. Table captions now state the summary statistic, which was the source of the discrepancy between Table~\ref{tab:headline_r1} and Table~\ref{tab:lastround}.}
...
\where{Sec.~\ref{sec:theory}: the theorem preamble now states the assumption limits, Corollary~2 of the submitted version is removed, and the Hessian measurement is reported in the same section.}
...
free.
\where{Table~\ref{tab:algos} in Sec.~\ref{sec:algos} (FedPAQ with top-$k$ at $k=10\%$, with byte accounting), and the SCAFFOLD non-result discussed in the same section.}
...
\where{Sec.~\ref{sec:lrsweep}, the paragraph on the seed-17 partition, with the ranking of all ten partitions and the rank correlations. The attribution sentence of the submitted version is removed.}
...
\reply We do not have a Raspberry~Pi, and we would rather say that than estimate a number for hardware
we cannot run. What we can measure honestly is the regime closest to it available to us:
single-thread CPU with batch size~1, on an idle machine. At matched parameters ($3{,}280$ against
$3{,}362$) FedKAN-8 costs $0.0995$~ms per sample against $0.0050$~ms for the parameter-matched
MLP, a factor of $19.9$, and $1.690$ against $0.045$~ms for a batch of $1000$, a factor of
$37.9$. Every KAN edge evaluates a B-spline where an MLP edge does one multiply-accumulate, so
the direction is expected; the magnitude is not what parameter parity would suggest.

Because a millisecond figure only describes the machine it was taken on, Table~\ref{tab:hwcost}
carries an anchor: one $256\times256$ matrix multiply on the same machine and thread takes
$0.024$~ms. A reader can measure that on their own gateway in a few lines and convert the rest.
Without such an anchor our numbers would not transfer, which is part of the reviewer's point.

We should also be explicit about what we withdrew rather than fixed. The submitted manuscript
reported per-round wall-clock of $1.8$--$2.5$~s for FedKAN against $1.5$--$2.1$~s for the MLP on
an RTX~4090 and described this as ``well within any IoT FL deployment budget''. That measurement
answers a different question, centralised training throughput on a workstation, and it is no
longer used to support any deployment claim. The submitted manuscript also stated that the model
``trains end-to-end on a Raspberry Pi-class device''. We never measured that, and the sentence is
gone.
\where{Sec.~\ref{sec:cost} and Table~\ref{tab:hwcost}. The RTX~4090 figures of the submitted version are no longer used for any deployment claim.}
...
capacity reading.
\where{Sec.~\ref{sec:lrsweep}, the sensitivity paragraph, with parameter counts alongside each configuration; the $G{=}10$ result is discussed in the seed-17 paragraph of the same section.}
...
\where{Sec.~\ref{sec:theory}, the paragraph ``An empirical check on the assumptions''.}
...
\where{Sec.~\ref{sec:lrsweep} in full, with Table~\ref{tab:lrsweep}, Table~\ref{tab:protocols} (four protocols side by side), Table~\ref{tab:lrrobust} and Table~\ref{tab:pooled}. The abstract, contributions and title all changed as a result.}
...
\reply Reported together with R2.3, and we summarise the outcome here because it is a negative one and
easy to miss in a cross-reference. We computed, for all ten Dirichlet partitions rather than for
seed~17 alone, the per-client sample-count Gini coefficient, the smallest and largest client, the
mean and minimum per-client label entropy, and the spread of the positive-class proportion across
clients. Ranking seed~17 against the other nine on each of these is the test: if it is not extreme
on any of them, then ``the partition is unusually skewed'' does not explain the failure.

It is not extreme. Seed~17 ranks second of ten by client-size Gini ($0.7235$), behind seed~31 at
$0.7600$, which behaves normally. It ranks ninth of ten by mean per-client entropy, and seed~29
is lower still while landing in the upper half of the accuracy ordering. Its positive-class spread
of $1.0$ is shared by eight of the ten partitions and so discriminates nothing. Rank correlations
against final accuracy are weak and not significant: Spearman $+0.42$ ($p=0.23$) for entropy,
$-0.30$ ($p=0.40$) for Gini, $+0.61$ ($p=0.06$) for the smallest client.

We therefore report the failure as reproducible but unexplained, and we removed the sentence in
the submitted version attributing it to an interaction between the Dirichlet partition and the
underlying class distribution. That sentence was an assertion; the measurements the reviewer asked
for do not support it. The reviewer's own hypothesis, that the B-spline grid support may not cover
some clients' feature range, remains untested and we say so rather than claim it.
\where{Sec.~\ref{sec:lrsweep}, same paragraph as R2.3.}
